package TestCase;

import static org.junit.Assert.*;

import org.junit.Test;

import cucumber.api.PendingException;
import cucumber.api.java.en.Given;
import cucumber.api.java.en.Then;
import cucumber.api.java.en.When;

public class RechercheViaCatalogueNominal {

	@Given("En tant qu utilisateur je souhaite pouvoir rechercher un produit de facon a pouvoir afficher le datail du produit via le catalogue")
	public void en_tant_qu_utilisateur_je_souhaite_pouvoir_rechercher_un_produit_de_facon_a_pouvoir_afficher_le_datail_du_produit_via_le_catalogue() {
	    // Write code here that turns the phrase above into concrete actions
	    throw new PendingException();
	}

	@When("Je veux naviguer dans le {string} puis sélectionner une {string} afin de sélectionner un {string}")
	public void je_veux_naviguer_dans_le_puis_sélectionner_une_afin_de_sélectionner_un(String string, String string2, String string3) {
	    // Write code here that turns the phrase above into concrete actions
	    throw new PendingException();
	}

	@When("Je veux voir le {string} sur produit pour l ajouter au panier")
	public void je_veux_voir_le_sur_produit_pour_l_ajouter_au_panier(String string) {
	    // Write code here that turns the phrase above into concrete actions
	    throw new PendingException();
	}

	@When("je verifie que {string} est ajoute au panier et que la {int} est la bonne ainsi que le {string}")
	public void je_verifie_que_est_ajoute_au_panier_et_que_la_est_la_bonne_ainsi_que_le(String string, Integer int1, String string2) {
	    // Write code here that turns the phrase above into concrete actions
	    throw new PendingException();
	}

	@When("Les verifications sont terminees je souhaite retirer {int}")
	public void les_verifications_sont_terminees_je_souhaite_retirer(Integer int1) {
	    // Write code here that turns the phrase above into concrete actions
	    throw new PendingException();
	}

	@Then("Je verifie le message {string} est present")
	public void je_verifie_le_message_est_present(String string) {
	    // Write code here that turns the phrase above into concrete actions
	    throw new PendingException();
	}

}
